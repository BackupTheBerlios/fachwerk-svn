...
package Fachwerk3D.addins3D.automModellsuche3D;
 ...
 * @author av
 */
public class clAutomModellsuche3D extends clElastisch implements inKonstante {
    ...
    protected double[] Ldy = new double[1 + anzSt];
    ...
    public clAutomModellsuche3D(clStab[] Staebe, clKnoten3D[] Kn, int[][] Top) {
        super(Staebe);
    ...
    private boolean optimiereSchritt(double[] verformungsenergie) {
        ...
        for (int st = 1; st < EA.length; st++) {
            ...
            if (Nst < 0) { // Druckstrebe
                ...
                // Lage des Stabes im Raum bestimmen
                boolean xzEbene = false; // dy=0
                boolean xyEbene = false; // dz=0, horizontal
                boolean yzEbene = false; // dx=0
                boolean xRtg = false;
                boolean yRtg = false;
                boolean zRtg = false;
                boolean diagonal = false; // dx=dy=dz
                double TOLlage = TOLgeom * L[st];
                // assert TOLlage > 0;
                if (Math.abs(Ldy[st]) < TOLlage) xzEbene = true;
                if (Math.abs(Ldz[st]) < TOLlage) xyEbene = true;
                if (Math.abs(Ldx[st]) < TOLlage) yzEbene = true;
                if (xzEbene && xyEbene) xRtg = true;
                if (xyEbene && yzEbene) yRtg = true;
                if (yzEbene && xzEbene) zRtg = true;
                if (Math.abs(Math.abs(Ldx[st]/L[st]) - Math.abs(Ldy[st]/L[st])) < TOLgeom // dx = dy
                    && Math.abs(Math.abs(Ldx[st]/L[st]) - Math.abs(Ldz[st]/L[st])) < TOLgeom) // dx = dz
                            diagonal = true;
                
                if (St[st].getStatus() == inKonstante.GESETZT) { // Stab gesetzter Zugstab
    ...
    public static void main(String[] args) throws Exception {
        ...
        clAutomModellsuche3D autom = new clAutomModellsuche3D(testSt, testKn, testTop);
