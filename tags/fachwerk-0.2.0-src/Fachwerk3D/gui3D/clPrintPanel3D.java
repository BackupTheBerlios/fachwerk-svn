 ...
 */
public class clPrintPanel3D extends clHauptPanel3D implements Printable {     // TODO
    ...
    public clPrintPanel3D(clKnoten3D[] p_Knotenarray, clWissenderStab3D[] p_Stabarray, boolean WIDERSPRUCHmelden, 
                            boolean FEHLERmelden, boolean KOMPLETTmelden, Locale lc) {
        ...
        //assert Math.abs(antGra + antSysgra + antSystxt - 1d) < 1e-10;  
    ...
    public int print(Graphics graphics, PageFormat pageformat, int pageIndex) throws PrinterException {
        ...
        zoomAll(false);
        koord.zoome(ZoomPkt1, ZoomPkt2, this, 1.5d * schriftgrSys); // maxPfeil);
        ...
        for (int kn = 1; kn < Kn.length; kn++) {
            switch (Kn[kn].getLagerbed()) {
                ...
                case VERSCHIEBLICH:
                    ...
                    //sb.append("(" + '\u03B1' + "=" + Fkt.fix(Math.toDegrees(Kn[kn].getRalpha()),1)+"°) "); // TODO
                    switch (Kn[kn].getLagerstatus()) {
                        case BER:
                            sb.append(Fkt.nf(Kn[kn].getRx(),1,5) + " ");
                            sb.append(Fkt.nf(Kn[kn].getRy(),1,5) + " ");
